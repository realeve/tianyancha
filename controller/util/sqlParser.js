...
function provinceList(provinces) {
    let sql = 'insert into provinceIndex(province,href,city) values '

    let sqlValues = provinces.map(item => {
        return `('${item.main}','${item.href}','${item.sub}')`;
    })
...
function industryList(industries) {
    let sql = 'insert into industryIndex(main,href,sub) values '

    let sqlValues = industries.map(item => {
        return `('${item.main}','${item.href}','${item.sub}')`;
    })
